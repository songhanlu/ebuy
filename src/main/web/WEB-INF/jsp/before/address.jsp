...
<head>
    ...
    <script type="text/javascript">
        /*$(function () {
            $("button[name=addAddressButton]").click(function () {
                $("#addAddressModel").model("show");
        ...
        $(function () {
            ...
            $("#addAddressSaveButton").click(function () {
                var address = $("#addAddressForm").serialize();
                $.post("${pageContext.request.contextPath}/userAdderss/queryAdd",
            ...
            //删除
            $("button[name=deleteAddress]").click(function () {
                var addressId = $(this).attr("addressId");
                if (confirm("确定要删除吗?")){
                $.post("${pageContext.request.contextPath}/userAdderss/deleteAddress",
            ...
            //修改
            $("button[name=updateAddress]").click(function () {
                var addressId = $(this).attr("addressId");
                $.post("${pageContext.request.contextPath}/userAdderss/queryAddressById",
                    {"id":addressId},function (data) {
                        $("#updateId").val(data.id);
                        $("#address").val(data.address);
                        $("#remark").val(data.remark);
                        $("#updateIsDefault").val(data.isDefault);
                    })
                $("#updateAddressModel").modal("show");
            })
            $("#updateAddressSaveButton").click(function () {
                var address=$("#updateAddressForm").serialize();
                $.post("${pageContext.request.contextPath}/userAdderss/updateAddress",
                    address,function (data) {
                        alert(data.message);
...
<div class="i_bg bg_color">
    ...
    <div class="m_content">
        ...
        <div class="m_right">
            <div class="btn-info" style="font-size: 20px; text-align: center">地址管理</div>
            <div style="text-align: right">
                <button type="button" class="btn btn-warning btn-sm" name="addAddressButton">添加</button>
            </div>
            ...
            <table class="table table-striped">
                ...
                <c:forEach items="${addresses}" var="address" varStatus="i">
                    <tr>
                        ...
                        <td><fmt:formatDate value="${address.createTime}" pattern="yyyy-MM-dd"/> </td>
                        <td>${address.remark}</td>
                        <td>
                            <c:if test="${address.isDefault eq 1}">是</c:if>
                            <c:if test="${address.isDefault ne 1}">否</c:if>
                        </td>
                        <td>

                            <button type="button" class="btn btn-info btn-sm" name="updateAddress" addressId="${address.id}">修改</button>
                            <button type="button" class="btn btn-danger btn-sm" name="deleteAddress" addressId="${address.id}">删除</button>
                        </td>
                    </tr>
                </c:forEach>
    ...
    <%--添加模态框--%>
    <div class="modal fade" tabindex="-1" role="dialog" id="addAddressModel">
            ...
            <div class="modal-content">
                ...
                <form class="form-horizontal" id="addAddressForm">
                    <div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <textarea class="form-control"  name="address" rows="3" placeholder="详细收获地址"></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <label class="radio-inline">
                            <input type="radio" name="isDefault" id="inlineRadio1" value="1" > 是
                        </label>
                        <label class="radio-inline">
                            <input type="radio" name="isDefault" id="inlineRadio2" value="0" checked> 否
                        </label>
                    </div>
                </form>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" class="btn btn-primary" id="addAddressSaveButton">保存</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
    <%--修改模态框--%>
    <div class="modal fade" tabindex="-1" role="dialog" id="updateAddressModel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                ...
                <form class="form-horizontal" id="updateAddressForm">
                    <div class="form-group">
                        <label  class="col-sm-2 control-label">收货地址</label>
                        <div class="col-sm-9">
                            <textarea class="form-control" id="address"  name="address" rows="3" placeholder="详细收获地址"></textarea>
                    ...
                    <div class="form-group">
                        <label  class="col-sm-3 control-label">是否默认</label>
                        <div class="col-sm-8">
                            <select class="form-control" id="updateIsDefault"  name="isDefault" >
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" class="btn btn-primary" id="updateAddressSaveButton">保存</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
